...
global {
	...
	init {
		...
		create building from: building_shapefile {
			if (shape.area < building_size.values[0]^2) {
				type <- wc_bd;
			} else if (shape.area < building_size.values[1]^2) {
				type <- uc_bd;
			} else if (shape.area < building_size.values[2]^2) {
				type <-app_bd;
			} else {
				type <- indu_bd ;
			}
	...
	action building_block_creation {
		...
		city <- union(building collect (each.shape + 20.0));
	}
...
experiment urbanevolution type: gui {
	output {
